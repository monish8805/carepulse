...
- **AccessRole management UI/endpoints beyond create + list:** editing a role's permissions and activating/deactivating a role. (Assigning a role to staff now happens as part of approving their access request — that part is built.)
- **Re-requesting after rejection**, and an admin endpoint to change an already-active staff member's `AccessRole`. Both are reasonable near-term additions, just not built yet.
