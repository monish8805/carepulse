...
- **One account = one hospital.** A user may hold at most one live (`pending`/`active`/`disabled`) `HospitalMembership`, enforced by an application-level check in `requestAccess`/`approveRequest`/`addStaffDirectly` plus a database-level partial unique index on `HospitalMembership.userId` as a backstop. Requesting or being added to a second hospital while one is held is rejected; becoming `removed`/`rejected`/`cancelled` frees the account to join a different hospital. Deliberately does not build the future multi-account "account switcher" — see PRD.md's Future scope.
...
- **`AccessRole` management: edit + delete ✅ Done.** `PATCH`/`DELETE /api/hospital/access-roles/:id` — editing changes permissions only (not the name); deletion is a hard delete, blocked while any active staff member holds the role (superseded the originally-planned activate/deactivate toggle — see CLAUDE.md's security-boundaries section for why). Create + list + assigning a role during approval already existed.
