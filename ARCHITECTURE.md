...
Distinct from the access-*request* lifecycle below: this is about already-`active` staff. `GET /api/hospital/staff` lists a hospital's active staff (name, email, their AccessRole's name); `DELETE /api/hospital/staff/:id` removes one (`domain/staff.service.ts::removeStaffMember`).

- **Authorization is broader than plain admin-gating**, on purpose: a hospital `admin` can always manage staff, but so can a staff member whose *current* `AccessRole` includes the `staff.manage` permission — resolved fresh via `resolvePermissions`, never cached, same as any other permission check. This is exposed to the frontend as `HospitalContext.canManageStaff` (computed in `domain/hospital.service.ts`, returned by `GET /me` and `POST /select`) — display/gating only, the backend re-derives and re-checks it independently on every actual staff-management request.
- **Peer protection:** a `staff.manage` holder cannot remove *another* `staff.manage` holder — only an admin can. This stops one manager-level staff member from removing another. Self-removal is blocked outright, for both admins and managers.
- **The hospital administrator can never be removed** through this endpoint — the target's `role` is checked to be `"staff"`, not `"admin"`, before anything else, regardless of who's asking.
- **Removal is a status change, not a hard delete:** the target `HospitalMembership.status` becomes `"removed"` (a new status alongside `pending`/`active`/`rejected` — see `models/hospitalMembership.model.ts`), so there's a record of who was staff and when they left. Like every other status, `verifyActiveMembership`/`resolvePermissions`/`getCurrentHospitalContext` all require `status: "active"`, so a removed member loses hospital access immediately (next request, no logout/refresh needed — same fail-closed-on-every-request principle as permission resolution).
- **Unlike `"rejected"`, `"removed"` is not terminal:** `requestAccess` lets a removed user submit a fresh request, reusing the same `HospitalMembership` document (the unique `(userId, hospitalId)` index allows only one) and resetting it to `"pending"` for an admin to decide again. This is a deliberately-designed transition (see CLAUDE.md: "state machines are one-way unless a transition is explicitly designed") — rejection stays terminal as before; only removal reopens the door.
...
- **Request** (`POST /api/hospital/access-requests`, body `{ hospitalId }`): any authenticated Hospital Portal session (`requireAuth` + `requirePortal("hospital")`, deliberately **no** current-hospital-context requirement, since a first-time requester has none) can request a specific hospital by id. Creates one `HospitalMembership{ role: "staff", status: "pending" }`. Fails with `404` if the hospital doesn't exist. If a membership already exists for that `(user, hospital)` pair: a `"removed"` one is reused and reset to `"pending"` (see "Staff management" above — a former staff member is allowed to ask again); any other existing status (`pending`/`active`/`rejected`) gets `409` — the model's unique index enforces one-document-per-pair at the database level regardless. **Grants nothing**: `verifyActiveMembership` and `resolvePermissions` both require `status: "active"`, so a pending (or rejected/removed) member cannot select that hospital or resolve any permission in it.
...
Still **Planned**: an admin endpoint to re-assign or change an already-active staff member's `AccessRole` outside of the initial approval, and re-requesting after a *rejection* specifically (still terminal — see PRD.md). Re-requesting after *removal* is different and already built — see "Staff management" above.
